(function ($) {
  ...
  function V4rnaRemoteModel() {
    ...
    function DataItem( row ) {
      ...
      this.pdb_zscorerms = row[5];
      this.curated_representative = row[6];
      this.curated_related = row[7];
      this.status = row[8];
      this.tm_packdens_protein_buried = row[9];
      this.tm_water_count = row[10];
      this.tm_residue_count = row[11];
      this.pdb_header = row[0];
      /*this.pdb_res = row[1];
      this.pdb_title = row[2];
      this.pdb_experiment = row[3];
      this.pdb_zscorerms = row[4];
      this.pdb_header = row[5];
    ...
    function ensureData(from, to, database) {
      if (req) {
        req.abort();
      ...
      searchstr = '';
      var url = "../query?keywds=" + searchstr + "&start=" + (fromPage * PAGESIZE) + "&limit=" + (((toPage - fromPage) * PAGESIZE) + PAGESIZE) + "&database=" + database;
  ...
  $.extend(true, window, {
    Slick: { Data: { V4rnaRemoteModel: V4rnaRemoteModel } }
  });
})(jQuery);
