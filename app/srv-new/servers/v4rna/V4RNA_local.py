...
def get_nop( app ):
    with sqlite3.connect( app.config['DATABASE'] ) as conn:
        c = conn.cursor()
        c.execute('SELECT COUNT(*) FROM VoronoiaDbRecord')
        nop = c.fetchone()[0]         
    return nop
...
def get_names( app ):
    with sqlite3.connect( app.config['DATABASE'] ) as conn:
        c = conn.cursor()
        c.execute('PRAGMA table_info(VoronoiaDbRecord)')
        names = c.fetchall()
...
@app.route('/', defaults={'page': "welcome"}, methods=['GET', 'POST'])
@app.route('/<string:page>/', methods=['GET', 'POST'])
def pages( page ):
    if page not in [
        "welcome", "grid", "calculation", "manual", "method",
        "statistics", "faq", "refs", "links", "wait", "results"
    ]:
        page = "welcome"
